...
def get_result(request, workflow_id: str):
    try:
        ...
        result = None
        is_complete = status and status.status in [
            "SUCCESS",
            "ERROR",
            "CANCELLED",
        ...
        return WorkflowResult(
            ...
            ),  # Use status.status to get the actual status string
            result=result,
            started=(
                datetime.fromtimestamp(status.created_at / 1000, tz=timezone.utc)
                if status and status.created_at
                else None
            ),
            completed=(
                datetime.fromtimestamp(status.updated_at / 1000, tz=timezone.utc)
                if is_complete and status and status.updated_at
                else None
            ),
        ...
        for wf in workflow_list:
            workflows.append(
                WorkflowInfo(
                    ...
                    created_at=(
                        datetime.fromtimestamp(wf.created_at / 1000, tz=timezone.utc)
                        if wf.created_at
                        else None
                    ),  # Convert from ms timestamp to UTC
                    updated_at=(
                        datetime.fromtimestamp(wf.updated_at / 1000, tz=timezone.utc)
                        if wf.updated_at
                        else None
                    ),  # Convert from ms timestamp to UTC
...
@router.get(
    "/status", response=WorkflowStatusResponse, summary="Get Workflow Status Overview"
)
def workflow_status(request, limit: int = 10000):
    ...
    try:
        ...
        return WorkflowStatusResponse(
            ...
            max_recovery_attempts_exceeded=status_counts[
                "MAX_RECOVERY_ATTEMPTS_EXCEEDED"
            ],
...
def get_workflow_details(request, workflow_id: str):
    ...
    try:
                    ...
                    WorkflowStepInfo(
                        ...
                        started_at=(
                            datetime.fromtimestamp(
                                step.created_at / 1000, tz=timezone.utc
                            )
                            if step.created_at
                            else None
                        ),
                        completed_at=(
                            datetime.fromtimestamp(
                                step.updated_at / 1000, tz=timezone.utc
                            )
                            if step.updated_at
                            else None
                        ),
        ...
        return WorkflowDetailResponse(
            ...
            app_version=(
                status.app_version
                if status and hasattr(status, "app_version")
                else None
            ),
            ...
            recovery_attempts=(
                ...
                if status and hasattr(status, "recovery_attempts")
                else 0
            ),
            steps=steps,
        )
